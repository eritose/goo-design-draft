package se.lvla.goobattle.web.rest;

import se.lvla.goobattle.config.KafkaProperties;
...
class GooDesignDraftKafkaResourceIT {
    ...
    void setup() {
        ...
        GooDesignDraftKafkaResource kafkaResource = new GooDesignDraftKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/goo-design-draft-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/goo-design-draft-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
